...
devenv --trace-output=trace.json shell
...
The replay tool reads JSON trace files generated by devenv's `--trace-output` flag or `--log-format=tracing-json` option and recreates the tracing events to visualize the operations in the TUI.
