...
let
  ...
  custom-nix = nix.packages.${pkgs.system}.nix;
  flake-file = import ./flake.nix { inherit pkgs version; };
  devenv-bin = pkgs.writeScriptBin "devenv" ''
    #!/usr/bin/env bash
