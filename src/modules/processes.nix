  ...
  options = {
    ...
    process.manager = {
      implementation = lib.mkOption {
        ...
        default =
          if config.devenv.cli.version != null && lib.versionAtLeast config.devenv.cli.version "2.0" then
            "native"
          else
            "process-compose";
    ...
    (lib.mkIf options.processes.isDefined (
      ...
      in
      {
        ...
        process.taskCommandsBase =
          ...
          lib.mapAttrs (
            name: _:
            "${config.task.package}/bin/devenv-tasks run --task-file ${config.task.config} --mode all --cache-dir ${lib.escapeShellArg config.devenv.dotfile} --runtime-dir ${lib.escapeShellArg config.devenv.runtime}${ignoreProcessDepsFlag} devenv:processes:${name}"
          ) enabledProcesses;

        # With exec prefix for proper signal handling (derived from base)
        process.taskCommands = lib.mapAttrs (name: cmd: "exec ${cmd}") config.process.taskCommandsBase;

        procfile = pkgs.writeText "procfile" (
          lib.concatStringsSep "\n" (
            lib.mapAttrsToList (name: _: "${name}: ${config.process.taskCommands.${name}}") enabledProcesses
          )
        );
