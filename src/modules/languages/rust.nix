...
in
{
  ...
  options.languages.rust = {
    ...
    mold.enable = lib.mkOption {
      ...
      defaultText = lib.literalExpression "pkgs.stdenv.isLinux && pkgs.stdenv.isx86_64 && languages.rust.targets == [ ]";
      description = ''
    ...
    lib.mkMerge [
      ...
      (lib.mkIf (cfg.channel != "nixpkgs") (
        let
          ...
          filteredToolchain = (
            lib.filterAttrs (n: _: builtins.elem n toolchain._manifest.profiles.complete) toolchain
          );
        in
        {
          languages.rust.toolchain = (builtins.mapAttrs (_: pkgs.lib.mkDefault) filteredToolchain);
