...

{
  ...
  config.lib = {
          ...
          if config.devenv.flakesIntegration then
            ''
              ...
              ${
                if follows != [ ] then
                  "inputs.${name}.inputs = { ${
                    lib.concatStringsSep "; " (map (i: "${i}.follows = \"${i}\"") follows)
                  }; };"
                else
                  ""
              }
            ''
          else if config.devenv.cli.version != null && lib.versionAtLeast config.devenv.cli.version "1.0" then
            ''
    ...
    getInput = args: inputs.${args.name} or (throw "${config.lib._mkInputError args}\n\n");
    ...
    getInputs =
      ...
      let
        ...
        check =
          if missingErrors != [ ] then throw (builtins.concatStringsSep "\n\n" missingErrors) else null;
      in
