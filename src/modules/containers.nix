...
let
  projectName =
    ...
    if config.name == null then
      throw ''You need to set `name = "myproject";` or `containers.${name}.name = "mycontainer"; to be able to generate a container.''
    else
      config.name;
  types = lib.types;
  ...
  homeRoots =
    cfg: (if (builtins.typeOf cfg.copyToRoot == "list") then cfg.copyToRoot else [ cfg.copyToRoot ]);
  ...
  mkDerivation =
    ...
    nix2container.nix2container.buildImage {
      ...
      layers =
        if cfg.enableLayerDeduplication then
          builtins.foldl' (
            layers: layer:
            layers
            ++ [
          ...
          builtins.map (layer: nix2container.nix2container.buildLayer layer) cfg.layers;
      ...
      config = {
        ...
        Cmd = if builtins.isList cfg.startupCommand then cfg.startupCommand else [ cfg.startupCommand ];
      };
    };
  ...
  mkCopyScript =
    ...
    '';
  containerOptions = types.submodule (
    { name, config, ... }:
    {
      options = {
        name = lib.mkOption {
