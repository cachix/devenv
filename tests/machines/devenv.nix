{ config, ... }: {
  ...
  # target work. This source intentionally does not exist.
  machines.missing-extra = {
    target.host = "root@missing-extra.example.com";
    install.extraFiles."/var/lib/missing-extra".source = "does-not-exist";
    nixos = {
      services.openssh.enable = true;
  ...
  # `machines install` must hide values but retain the effective default
  # profile, producing the valid 0600 mode.
  machines.secretful = {
