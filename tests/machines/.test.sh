...
for m in empty hm mac missing-extra missing-secret nixos nohost secretful; do
...
# 18a. A local-mode install with no bootstrap secrets does not resolve the
# entire SecretSpec profile just to discover Cachix authentication. An existing
# environment token skips SecretSpec altogether, so the unrelated required
# bootstrap key can be absent and the install reaches its build.
if env -u SECRET_MACHINE_AGE_KEY CACHIX_AUTH_TOKEN=already-present \
  devenv machines install --phases install nixos 2>err.log; then
  echo "expected missing-disko failure after skipping SecretSpec"
  exit 1
fi
grep -q "devenv inputs add disko" err.log
if grep -q "Missing required secrets: SECRET_MACHINE_AGE_KEY" err.log; then
  echo "existing Cachix authentication triggered whole-profile SecretSpec resolution"
  cat err.log
  exit 1
fi

# 18b. With no environment token, the omitted Cachix setting performs one
# optional named lookup. CACHIX_AUTH_TOKEN is undeclared in this fixture, which
# must not make the unrelated required bootstrap key block the install.
if env -u SECRET_MACHINE_AGE_KEY -u CACHIX_AUTH_TOKEN \
  devenv machines install --phases install nixos 2>err.log; then
  echo "expected missing-disko failure after optional Cachix lookup"
  exit 1
fi
grep -q "devenv inputs add disko" err.log
if grep -q "Missing required secrets: SECRET_MACHINE_AGE_KEY" err.log; then
  echo "optional Cachix lookup validated unrelated SecretSpec declarations"
  cat err.log
  exit 1
fi
...
# 27a. Missing extra-file payloads fail locally before the NixOS build or any
# target interaction. The fixture's missing disko input is a sentinel for a
# build that happened too early.
if devenv machines install --phases install missing-extra 2>err.log; then
  echo "expected missing extra-file source to fail"
  exit 1
fi
grep -q "Failed to read extra file" err.log
if grep -q "devenv inputs add disko\|Preflight probe failed" err.log; then
  echo "missing extra-file source was checked after target work began"
  cat err.log
  exit 1
fi
