...
# `start.shell = true` with a non-native process manager must fail fast with a
# clear assertion error rather than silently misbehaving.
...
echo "PASS: start.shell = true rejected with non-native process manager"
