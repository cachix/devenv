...
let
  ...
  dotenv = import ./../../src/modules/integrations/dotenv-functions.nix { lib = nixpkgs.lib; };
in
runTests {
  testIsEven_1 = {
    expr = math.isEven 2;
  ...
  #     (u"", []),
  testEmptyConfigEmpty = {
    expr = dotenv.parseEnvFile "";
    expected = { };
  };
  #     (u"a=b", [Binding(key=u"a", value=u"b", original=Original(string=u"a=b", line=1), error=False)]),
  testSimpleAB = {
    expr = dotenv.parseEnvFile "a=b";
    expected = { "a" = "b"; };
  };
  # https://github.com/cachix/devenv/issues/772
  testJsonQuotedCorrectly = {
    expr = dotenv.parseEnvFile "a='{\"a\":\"b\"}'";
    expected = { "a" = "{\"a\":\"b\"}"; };
  };
