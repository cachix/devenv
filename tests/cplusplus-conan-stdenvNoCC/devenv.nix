...
in
{
  ...
  assertions = [
    ...
    {
      assertion = cfg.conan.config.stdenv == pkgs.stdenv;
      message = "languages.cplusplus.conan.config.stdenv == pkgs.stdenv when languages.cplusplus.conan.enable and devenv is configured with stdenv = pkgs.stdenvNoCC.";
    }
    {
