    ...
    pub fn register_all(&self, eval_state: &mut EvalState) -> Result<Value> {
        ...
        let values = self
            ...
            .map(|registration| {
                ...
                registration.register(eval_state)
            })
...
pub struct LoadDotenvPrimop {
    root: PathBuf,
    tracker: Arc<DotenvTracker>,
}

impl LoadDotenvPrimop {
    pub fn new(root: PathBuf, tracker: Arc<DotenvTracker>) -> Self {
        Self { root, tracker }
    }
}
    ...
    fn register(&self, eval_state: &mut EvalState) -> Result<(String, Value)> {
        ...
        let primop = PrimOp::new(
            ...
            Box::new(move |es, [filenames, substitution]| {
                ...
                let variables = load_dotenv_tracked(&paths, substitution, &tracker)
