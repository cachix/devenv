...
pub enum CommandError {
    ...
    Io(#[from] io::Error),
    #[error(transparent)]
    Cache(#[from] devenv_cache_core::error::CacheError),
}
...
impl<'a> NixCommand<'a> {
    ...
    pub async fn output(mut self, cmd: &'a mut Command) -> Result<Output, CommandError> {
        ...
        if self.enable_caching
            && !self.force_refresh
            && let Ok(Some(output)) =
                query_cached_output(self.conn, &cmd_hash, &self.extra_paths).await
        {
            return Ok(output);
        }
...
async fn query_cached_output(
    ...
    let cached_cmd = db::get_command_by_hash(conn, cmd_hash).await?;
    ...
    if let Some(cmd) = cached_cmd {
        ...
        let files = db::get_files_by_command_id(conn, cmd.id).await?;

        let envs = db::get_envs_by_command_id(conn, cmd.id).await?;
                    ...
                    match file_state {
                        FileState::MetadataModified { modified_at, .. } => {
                            if let Input::File(file) = &inputs[index] {
                                ...
                                db::update_file_modified_at(conn, &file.path, modified_at).await?;
                            }
                        }
                        FileState::Modified {
                            new_hash,
        ...
        if should_refresh {
            ...
            db::update_command_updated_at(conn, cmd.id).await?;
