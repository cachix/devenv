...
- Managed files now report destinations they cannot safely replace, convert an unchanged `copy` materialization back to a symlink when `copyMode` changes, and reclaim private replacement artifacts after reconciliation state is lost.
- Managed files migrate legacy absolute-path state (including Claude settings), honor task command overrides, and replace FIFO and socket destinations without blocking. Native tasks now participate in `execIfModified` caching.
...
- Managed project files are reconciled natively when the CLI advertises an exact compatible builtin version, avoiding file and state rewrites on unchanged runs. For safer descriptor-relative updates, destination paths may no longer pass through symlinked parent directories. Writable copies preserve source POSIX modes with owner-write enabled rather than applying the caller's umask.
- In benchmarks with 256 managed files, native reconciliation is 10–56× faster on initial creation and about 169× faster on unchanged runs than mainline's shell cleanup/create tasks on macOS/APFS. On Linux/Btrfs, creation is 2.3–10.5× faster and unchanged runs are 70–76× faster ([benchmark details](devenv-tasks/tests/FILESYSTEMS.md)).
- Managed file destinations must stay within the project root on both native and older shell runners. Root-contained absolute paths remain compatible but warn in favor of reproducible relative paths; equivalent relative spellings are normalized, and parent traversal is rejected.
