...
- Fixed shell hook exiting into a stale directory when a prior `.devenv/exit-dir` cleanup had failed (for example because the user's shell aliases `rm` to something that does not actually delete). Each hook activation now stamps a unique nonce, and any exit-dir whose nonce does not match is ignored.
