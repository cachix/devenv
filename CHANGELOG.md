...
- Fixed `devenv` invocations with different `$TMPDIR` values (e.g. commands run by sandboxed tools like Claude Code, or under `nix develop`) not finding each other's running processes, leading to orphaned processes and duplicate `devenv up` instances. The runtime directory no longer depends on `$TMPDIR`, and a `$DEVENV_RUNTIME` inherited from a shell of the same project is now validated and reused. The runtime directory is also now created with owner-only permissions ([#2923](https://github.com/cachix/devenv/issues/2923)).
