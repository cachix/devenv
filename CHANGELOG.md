...
- Fixed `devenv.yaml` `imports:` silently dropping a directory that was imported transitively (through another project's `devenv.yaml`) rather than directly by the root project, when that directory had no `devenv.yaml` of its own. Its `devenv.nix` is now loaded regardless of how deep the import chain is.
- Fixed editor-launched terminals and stripped environments defaulting to Bash when `$SHELL` is absent. devenv now falls back to the user's login shell (resolved via `getpwuid(getuid())` — no environment variables required, correct under `sudo`/`su`) before defaulting to Bash. The login shell's absolute path is threaded through both the PTY and non-PTY interactive paths so devenv can exec the correct shell even when `$PATH` does not contain its Nix store directory. A stale passwd entry whose path no longer exists on disk gracefully falls back to the previous `resolve_shell_path` logic. devenv also now warns when a shell explicitly requested via `--shell` or `devenv.yaml` is unsupported and silently downgraded to Bash ([#2880](https://github.com/cachix/devenv/issues/2880), [#2992](https://github.com/cachix/devenv/pull/2992)).
- Fixed the shell hook closing a terminal multiplexer pane/session (e.g. tmux, zellij) when `cd`-ing out of a project directory inside it. `DEVENV_ROOT`/`_DEVENV_HOOK_DIR` are process environment variables and were inherited by any shell started from within an active devenv shell (a new pane, an ssh session, a manually started nested shell); such shells wrongly concluded they were the hook-spawned one and `exit`ed on cd-out with nothing around to catch it. The marker is now consumed and removed from the environment as soon as each shell picks it up, so it can no longer leak to processes started afterward ([#2861](https://github.com/cachix/devenv/issues/2861)).
