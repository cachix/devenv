...
- Fixed the shell hook closing a terminal multiplexer pane/session (e.g. tmux, zellij) when `cd`-ing out of a project directory inside it. `DEVENV_ROOT`/`_DEVENV_HOOK_DIR` are process environment variables and were inherited by any shell started from within an active devenv shell (a new pane, a manually started nested shell); such shells wrongly concluded they were the hook-spawned one and `exit`ed on cd-out with nothing around to catch it. The marker is now consumed and removed from the environment as soon as each shell picks it up, so it can no longer leak to processes started afterward ([#2861](https://github.com/cachix/devenv/issues/2861)).
- Fixed `cd`-ing out of a devenv-hook-activated project not reliably deactivating the shell outside terminal multiplexers. Detecting cd-out depended on the spawned shell's own rc file re-sourcing the hook script a second time, which some shell configs skip (e.g. a fish config gated behind `status is-login`, which a hook-spawned non-login `fish -i` never satisfies) — silently leaving the stale environment active. Each shell dialect's own generated init file now handles cd-out detection directly, independent of the user's rc file.
- Fixed a devenv shell being redundantly spawned on top of an environment that direnv (or another tool) already activated for the same directory via `use devenv` in `.envrc`. The fish hook decides whether to activate on one prompt but only acts on the next (deliberately, to avoid spawning mid cd), and didn't recheck in between whether something else had activated the directory first.
- Fixed `cd -` not returning to the project directory after the fish hook followed you out of it on cd-out ([#2853](https://github.com/cachix/devenv/issues/2853)). That follow-cd uses `builtin cd` rather than `cd` to avoid re-triggering a user-overridden `cd` (e.g. `zoxide init --cmd=cd`, which otherwise reports "infinite loop detected"), but `builtin cd` also skips fish's own directory-history bookkeeping, which lives in its `cd` function rather than a variable-change hook — so `cd -` silently skipped over the project directory. The follow-cd now replicates that bookkeeping directly.
- A terminal opened straight into a trusted project directory (e.g. a fresh Zed/tmux tab) now closes with a single `exit`/Ctrl-D instead of two. The outer shell never showed the user its own prompt before auto-activating, so once the inner devenv shell exits (not a cd-out), there's no prior state left to preserve and the exit now propagates. A shell that was already in use before entering the project still requires the usual two, since it may be wanted back.
- Fixed a hook-spawned shell wrongly exiting on cd-out (and closing the pane/session) while the user was still inside the project, when the project lives under a symlinked path (e.g. macOS's `/tmp` -> `/private/tmp`). `$PWD` preserves the symlink the user actually navigated through while `DEVENV_ROOT` is canonicalized, so comparing the two as raw strings could spuriously conclude the user had left. Both sides are now resolved (via builtins only — `path resolve` in fish, `${PWD:A}`-equivalent `cd -P`/`pwd` in bash/zsh, `path expand` in nu — no `realpath` dependency) before comparing.
- Fixed automatic shell-hook activation spawning the wrong shell dialect's environment when `$SHELL` didn't match the shell the hook was actually loaded into (e.g. a fish user whose `$SHELL` was still `/bin/bash`). Each hook script now tells `devenv shell` explicitly which dialect to spawn instead of leaving it to devenv's `$SHELL` fallback.
- Clarified the `devenv shell --shell nu`/`fish`/`zsh` "shell not found" error: it used to read as if any valid `$SHELL` would satisfy it, when it specifically means pointing `SHELL` at *that dialect's* own absolute path.
- Fixed `devenv shell` for bash silently skipping hook-dir/cd-out/symlink handling outside the TUI (e.g. non-interactive terminals, `--no-tui`, CI). That path used an older, separate script generator that predates those fixes; it now goes through the same generation as zsh/fish/nu.
- Fixed `cd -` not returning anywhere right after a devenv shell first activates (hook-triggered or manual). Bash/zsh discard an inherited `OLDPWD` unconditionally, and fish's `cd -` doesn't consult `OLDPWD` at all (it tracks `$dirprev`, which doesn't cross a process boundary either); nu is the only one of the four that just works if `OLDPWD` is set. Each hook script now passes along where you were before activating, and each dialect re-derives its own equivalent on startup.
- Fixed `cd -` being lost (fish only) specifically when a project was reactivated by typing `cd -` itself, rather than a fresh `cd <name>`. Fish's `cd -` moves the entry the hook needs from `$dirprev` to `$dirnext`, and the hook was only ever looking at `$dirprev`.
- Fixed the bash/zsh hook silently failing to reactivate when `cd`-ing back into the *same* project right after leaving it. Following the user out on cd-out never updated the hook's "did the directory change since I last checked" cache, so it stayed stale at the project directory — coincidentally matching again on re-entry and skipping the check entirely.
- Fixed input-relative imports (e.g. `imports: [myinput/some/dir]`) being wrongly rejected with "Import path resolves outside the git repository" when the project root is reached through a symlink, such as macOS temp directories under `/var`.
