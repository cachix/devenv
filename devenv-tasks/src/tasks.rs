...
impl Tasks {
    ...
    pub async fn run(&self) -> Outputs {
        ...
        for index in &self.tasks_order {
            ...
            // Capture orchestration activity ID for parenting across spawn boundary
            let orchestration_id = orchestration_activity.id();

            running_tasks.spawn(move || async move {
                // Run the task within the orchestration activity's parent context
                // so child task activities have proper parent-child relationships
                devenv_activity::with_parent(orchestration_id, async {
                    let completed = {
                    ...
                    let done = completed_tasks_clone
                        ...
                        + 1;
                    orchestration_activity_clone.progress(done, total_tasks);
