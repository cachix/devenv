...
async fn process_status_watcher(
    ...
    // and watch until a terminal phase (GaveUp) or the channel closes (process
    // stopped). When the channel closes we set Stopped and loop back so a
    // subsequent start/restart re-subscribes automatically.
    loop {
        let notified = notify_finished.notified();
        tokio::pin!(notified);
        notified.as_mut().enable();
