...
mod tests {
    ...
    #[tokio::test]
    async fn bounded_backlog_and_live_tail_join_partial_lines_without_duplicates() {
        let temp_dir = tempfile::tempdir().unwrap();
        let path = temp_dir.path().join("stdout.log");
        std::fs::write(&path, b"old-one\nold-two\npartial").unwrap();

        let (backlog, offset) = read_backlog(&path, 1);
        assert_eq!(backlog, ["old-two"]);
        assert_eq!(offset, b"old-one\nold-two\n".len() as u64);

        let cancel = CancellationToken::new();
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel();
        let tailer = spawn_tail_to(path.clone(), offset, true, cancel.clone(), move |line| {
            tx.send(line).is_ok()
        });
        ...
        let first = tokio::time::timeout(Duration::from_secs(30), rx.recv())
            .await
            .expect("timed out waiting for completed partial line")
            .expect("tailer closed");
        let second = tokio::time::timeout(Duration::from_secs(30), rx.recv())
            .await
            .expect("timed out waiting for live line")
            .expect("tailer closed");
        assert_eq!(first, "partial-done");
        assert_eq!(second, "live");
        assert!(
            rx.try_recv().is_err(),
            "backlog lines must not be emitted again by the live tail"
        );
    ...
    #[test]
    fn backlog_is_bounded_and_strips_carriage_returns() {
        let temp_dir = tempfile::tempdir().unwrap();
