...
impl ChangelogEntry {
    /// Generate a unique hash for this changelog entry
...
impl<'a> Changelog<'a> {
    ...
    async fn load_changelogs(&self) -> Result<Vec<ChangelogEntry>> {
        let changelog_json_file = {
            let gc_root = self.dot_gc.join("changelog-data");
            self.nix
                .build(&["devenv.config.changelog.data"], None, Some(&gc_root))
                .await?
        };
    ...
    fn display_changelogs(&self, changelogs: &[ChangelogEntry]) -> Result<()> {
        ...
        for entry in changelogs {
            // Format: (date) with affects on same line if present
            let mut header = format!("**{}**", entry.date);
            if !entry.affects.is_empty() {
                header.push_str(entry.affects.join(", "));
            }
