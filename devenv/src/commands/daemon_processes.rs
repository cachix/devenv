...
pub fn run(config_file: &Path) -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        // before `run_with_parent_activity` returns). Without the handler set,
        // an `Up` arriving mid-startup is rejected outright; buffered on this
        // channel it instead waits until the foreground loop drains it once
        // startup completes.
