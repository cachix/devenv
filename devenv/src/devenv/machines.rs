...
use devenv_core::{cachix::CACHIX_AUTH_TOKEN_ENV, settings::SecretSettings};
...
const MACHINE_EXECUTOR: &str =
    "/nix/var/nix/gcroots/devenv-machines/executor/bin/devenv-machine-deploy";

// Observe a fixed running generation, and reject an observable concurrent
// profile/switch change. No helper installation or target mutation is needed.
const MACHINE_PLAN_OBSERVATION: &str = r#"set -eu
running=$(readlink -e /run/current-system)
profile=$(readlink -e /nix/var/nix/profiles/system)
printf 'devenv-machine-plan-v1\n%s\n%s\n' "$running" "$profile"
nix-store --query --requisites "$running"
test "$(readlink -e /run/current-system)" = "$running"
test "$(readlink -e /nix/var/nix/profiles/system)" = "$profile"
"#;

fn validate_plan_store_path(path: &str) -> Result<()> {
    let valid = path
    ...
    if !valid {
        bail!("Invalid store path in deployment preview: {path:?}");
    }
    Ok(())
}

fn parse_plan_observation(output: &str) -> Result<(String, String, BTreeSet<String>)> {
    let mut lines = output.lines();
    if lines.next() != Some("devenv-machine-plan-v1") {
        bail!("Invalid target response for deployment preview");
    }
    let running = lines
        .next()
        .ok_or_else(|| miette!("Missing running system in deployment preview"))?;
    let profile = lines
        .next()
        .ok_or_else(|| miette!("Missing system profile in deployment preview"))?;
    validate_plan_store_path(running)?;
    validate_plan_store_path(profile)?;
    let mut closure = BTreeSet::new();
    for path in lines {
        validate_plan_store_path(path)?;
        closure.insert(path.to_owned());
    }
    if !closure.contains(running) {
        bail!("Target closure is missing its running system; deployment preview is incomplete");
    }
...
type InstallFileValues = HashMap<String, SecretSlice<u8>>;
...
///
/// Accepts `user@host`, `user@host:port`, `user@[ipv6]`,
/// `user@[ipv6]:port`, and `ssh://user@host:port`.
...
/// A conservative identity for the physical host behind an SSH target.
///
/// User and port are deliberately excluded: two machine entries that use
/// different SSH accounts or endpoints can still reach the host whose disks
/// an install is replacing.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
struct MachineHostIdentity(String);
...
impl SshTarget {
    pub(crate) fn parse(input: &str) -> Result<Self> {
        ...
        let (host, port) = if let Some(after_open_bracket) = after_user.strip_prefix('[') {
            let closing_bracket = after_open_bracket.find(']').ok_or_else(|| {
                miette!("target.host '{input}' has an unterminated bracketed host component")
            })?;
            let address = &after_open_bracket[..closing_bracket];
            if address.is_empty() {
                bail!("target.host '{input}' has an empty host component");
            }
            let suffix = &after_open_bracket[closing_bracket + 1..];
            let port = match suffix {
                "" => None,
                _ => Some(parse_ssh_port(
                    input,
                    suffix.strip_prefix(':').ok_or_else(|| {
                        miette!(
                            "target.host '{input}' has invalid content after its bracketed host component"
                        )
                    })?,
                )?),
            };
            ...
            match after_user.rsplit_once(':') {
                Some((h, p)) => {
                    let port = parse_ssh_port(input, p)?;
    ...
    fn host_identity(&self) -> MachineHostIdentity {
        ...
        let normalized = unbracketed
            ...
            .unwrap_or_else(|_| unbracketed.trim_end_matches('.').to_ascii_lowercase());
        MachineHostIdentity(normalized)
    }
}

fn selected_host_identities(
    meta: &BTreeMap<String, MachineMeta>,
    names: &[String],
) -> Result<HashSet<MachineHostIdentity>> {
    names
        .iter()
        .map(|name| {
            let host = meta[name]
                ...
                .host
                .as_deref()
                .expect("selected install machines have a target.host");
            SshTarget::parse(host)
                .map(|target| target.host_identity())
...
fn load_local_machine_secretspec(
    devenv_root: &Path,
    secret_settings: &SecretSettings,
) -> Result<Option<secretspec::Secrets>> {
    let Some(config) = secret_settings
        .secretspec
        .as_ref()
    ...
    let path = devenv_root.join("secretspec.toml");
    if !path.exists() {
        return Ok(None);
    }

    let mut secrets = secretspec::Secrets::load_from(&path)
        .map_err(|error| miette!("Failed to load secretspec configuration: {error}"))?;
...
fn resolved_secret_bytes(
    name: &str,
    resolved: secretspec::ResolvedSecret,
) -> Result<SecretSlice<u8>> {
    match (resolved.value, resolved.path) {
        (Some(value), None) => Ok(SecretSlice::from(value.into_bytes())),
        (None, Some(path)) => std::fs::read(&path)
            .into_diagnostic()
            .wrap_err_with(|| format!("Failed to read temporary file for SecretSpec secret {name}"))
            .map(SecretSlice::from),
        _ => Err(miette!(
            "SecretSpec returned an invalid resolved value for secret {name:?}"
        )),
    }
}

fn should_resolve_opportunistic_cachix_token(
    secret_settings: &SecretSettings,
    needs_local_build_auth: bool,
    has_local_machine: bool,
    environment_has_token: bool,
) -> bool {
    needs_local_build_auth
        && has_local_machine
        && !environment_has_token
        && secret_settings
            .secretspec
            .as_ref()
            .is_some_and(|config| config.enable && config.cachix_auth_token.is_none())
}
...
/// Reject repeated machine names before any target interaction. Running two
/// destructive pipelines for the same machine is never meaningful, even when
/// concurrency is explicitly limited to one.
fn validate_unique_machine_names(names: &[String], operation: &str) -> Result<()> {
    ...
    if !duplicates.is_empty() {
        bail!(
            "Duplicate machine name(s) in {operation} request: {}. Each machine may be named only once.",
            duplicates.join(", ")
        );
    }
    Ok(())
...
fn parse_ssh_port(input: &str, port: &str) -> Result<u16> {
    port.parse().map_err(|_| {
        miette!("target.host '{input}' has an invalid port: {port:?}. Expected a number 1-65535.")
    })
}
...
impl Devenv {
    ...
    pub async fn machines_install(
        ...
        validate_unique_machine_names(names, "install")?;
        let meta = self.load_machines_meta().await?;
        ...
        // Read every local payload before starting any per-machine job so a
        // missing or unreadable file fails before preflight, kexec, or disko
        // touches a target. SecretSlice keeps potentially sensitive key and
        // configuration bytes out of debug output and core dumps.
        let mut install_file_values = InstallFileValues::new();
        for name in names {
            let machine = &meta[name];
            let sources = machine
                .extra_files
                .values()
                .filter(|_| phases.contains(&InstallPhase::Install))
                .map(|file| (&file.source, "extra file"))
                .chain(
                    machine
                        .encryption_keys
                        .values()
                        .filter(|_| phases.contains(&InstallPhase::Disko))
                        .map(|source| (source, "encryption key")),
                );
            for (source, kind) in sources {
                if install_file_values.contains_key(source) {
                    continue;
                }
                let contents = tokio::fs::read(source)
                    ...
                    .into_diagnostic()
                    .wrap_err_with(|| {
                        format!("Failed to read {kind} {source:?} for machines.{name}")
                    })?;
                install_file_values.insert(source.clone(), SecretSlice::from(contents));
            }
        }

        // Prepare every bootstrap input before starting any per-machine job.
        // Local machines resolve values now. Target machines receive only a
        // self-contained manifest and do not contact the local provider.
        let mut bootstrap_values = HashMap::new();
        let mut target_manifests = TargetBootstrapManifests::new();
        let has_local_machine = names
            .iter()
            .any(|name| meta[name].secretspec.execution == SecretspecExecution::Local);
        let needs_local_build_auth =
            phases.contains(&InstallPhase::Disko) || phases.contains(&InstallPhase::Install);
        let mut local_secret_references: BTreeMap<String, Vec<String>> = BTreeMap::new();
        if phases.contains(&InstallPhase::Install) {
            for name in names {
                let machine = &meta[name];
                if machine.secretspec.execution != SecretspecExecution::Local {
                    continue;
                }
                for secret in &machine.bootstrap_secrets {
        ...
        // The project token is deliberately opportunistic when the setting is
        // omitted. An environment token wins without loading SecretSpec, while
        // explicit true/string settings were already handled by the built-in
        // Cachix manifest during Devenv construction.
        let resolve_project_cachix_token = should_resolve_opportunistic_cachix_token(
        ...
        let mut requested_names: BTreeSet<String> =
            local_secret_references.keys().cloned().collect();
        if resolve_project_cachix_token {
            requested_names.insert(CACHIX_AUTH_TOKEN_ENV.to_string());
        }

        // Resolve exactly what this install consumes. `resolve_named` includes
        // composition dependencies but does not validate or contact providers
        // for unrelated declarations in the active profile.
        if !requested_names.is_empty() {
            let mut secrets =
                load_local_machine_secretspec(&self.devenv_root, &self.options.secret_settings)?;
            if secrets.is_none() && !local_secret_references.is_empty() {
                bail!("Local machine bootstrap secrets require an enabled secretspec.toml");
            }

            if let Some(secrets) = secrets.as_mut() {
                let mut resolved_values = HashMap::new();
                let mut missing_required = Vec::new();
                let mut undeclared_references = Vec::new();

                for secret_name in requested_names {
                    match secrets.resolve_named(&secret_name).map_err(|error| {
                        miette!("Failed to resolve SecretSpec secret {secret_name:?}: {error}")
                    })? {
                        secretspec::NamedResolution::Resolved(resolved) => {
                            resolved_values.insert(
                                secret_name.clone(),
                                resolved_secret_bytes(&secret_name, resolved)?,
                            );
                        }
                        secretspec::NamedResolution::Missing { .. } => {
                            if local_secret_references.contains_key(&secret_name) {
                                missing_required.push(secret_name);
                            }
                        }
                        secretspec::NamedResolution::Undeclared => {
                            if let Some(references) = local_secret_references.get(&secret_name) {
                                undeclared_references.extend(references.iter().cloned());
                            }
                            // An omitted Cachix setting means "use it if this
                            // project declares it", so undeclared is expected.
                        }
                    }
                }
                ...
                if !undeclared_references.is_empty() {
                    bail!(
                        ...
                        undeclared_references.len(),
                        undeclared_references.join("\n- ")
                    );
                }
                if !missing_required.is_empty() {
                    missing_required.sort();
                    missing_required.dedup();
                    let config = self.options.secret_settings.secretspec.as_ref();
                    return Err(super::SecretsNeedPrompting {
                        provider: config.and_then(|config| config.provider.clone()),
                        profile: config.and_then(|config| config.profile.clone()),
                        missing: missing_required,
                        source: super::SecretsPromptSource::Project,
                    }
                    .into());
                }

                if resolve_project_cachix_token
                    && let Some(token) = resolved_values.get(CACHIX_AUTH_TOKEN_ENV)
                {
                    let token = String::from_utf8(token.expose_secret().to_vec())
                        .into_diagnostic()
                        .wrap_err("SecretSpec CACHIX_AUTH_TOKEN is not valid UTF-8")?;
                    if !token.is_empty() {
                        self.cachix_manager.set_auth_token_override(Some(token));
                    }
                }

                for secret_name in local_secret_references.keys() {
                    let value = resolved_values.remove(secret_name).ok_or_else(|| {
                        miette!("SecretSpec did not resolve bootstrap secret {secret_name:?}")
                    })?;
                    bootstrap_values.insert(secret_name.clone(), value);
                }
            }
        }
        ...
        // An install target stops being a usable builder as soon as its job
        // kexecs. Exclude the entire working set before any concurrent build
        // can be dispatched to a machine that another job is replacing.
        if use_machines_as_builders {
            let install_targets = selected_host_identities(&meta, names)?;
            configure_remote_builders(self, &meta, &install_targets)?;
        }
        ...
        let jobs = names.iter().map(|name| {
            ...
            let install_file_values = &install_file_values;
    ...
    async fn install_one_machine(
        ...
        install_file_values: &InstallFileValues,
    ) -> Result<()> {
        let host = machine
            .target
            .host
        ...
        if phases.contains(&InstallPhase::Disko) && !machine.encryption_keys.is_empty() {
            ...
            async {
                self.install_encryption_keys(
                    &target,
                    &ssh_opts,
                    &machine.encryption_keys,
                    install_file_values,
                )
                .await
            }
        ...
        // bootstrap payloads after it finishes so the installer cannot
        // overwrite them and they are present for the first real boot.
        if phases.contains(&InstallPhase::Install) && !machine.extra_files.is_empty() {
            let act = activity!(INFO, operation, "Copying extra files");
            async {
                self.install_extra_files(
                    &target,
                    &ssh_opts,
                    &machine.extra_files,
                    install_file_values,
    ...
    async fn install_encryption_keys(
        ...
        for (target_path, local_source) in keys {
            let contents = install_file_values.get(local_source).ok_or_else(|| {
                miette!("Encryption key {local_source:?} was not prepared before installation")
            })?;
    ...
    async fn install_extra_files(
        ...
        for (target_path, file) in files {
            let contents = install_file_values.get(&file.source).ok_or_else(|| {
                miette!(
                    "Extra file {:?} was not prepared before installation",
                    file.source
                )
            })?;
    ...
    pub async fn machines_deploy_reviewed(
        ...
        let selected: Vec<String> = if names.is_empty() {
            meta.iter()
                .filter(|(_, machine)| machine.target.host.is_some())
        ...
        for name in &selected {
            if !meta.contains_key(name) {
                bail!(
                    ...
                    meta.keys().cloned().collect::<Vec<_>>().join(", ")
                );
            }
        }
        ...
        if !yes {
            if !crate::terminal::can_use_stdin_interactively() {
                bail!(
                    "Deployment needs confirmation. Run interactively, pass --yes, or review and apply saved plan {id}"
                );
            }
            if !dialoguer::Confirm::new()
                ...
                .into_diagnostic()?
            {
                eprintln!("Deployment cancelled. Saved plan: {id}");
                return Ok(());
            }
        }
    ...
    pub async fn machines_saved_plan(&self, names: &[String], json: bool) -> Result<String> {
        let contents = self.machines_plan(names).await?;
        let plan = MachinePlan::parse(&contents)?;
        let id = self.save_machine_plan(&contents, &plan).await?;
        if json {
            return Ok(contents);
        }
        Ok(format!(
            "{}\nSaved plan: {id}\nApply with: devenv machines apply {id}\n",
            plan.summary()
        ))
    }

    async fn save_machine_plan(&self, contents: &str, plan: &MachinePlan) -> Result<String> {
        use std::io::Write;
        let directory = self.dotfile().join("machine-plans");
        ...
        let saved = tempfile::Builder::new()
            ...
            .into_diagnostic()?;
        let mut file = std::fs::File::create(saved.path().join("plan.json")).into_diagnostic()?;
        ...
        for (name, entry) in &plan.machines {
            for (role, package) in [
                ...
                self.backend()
                    .store()
                    .add_gc_root(
                        &saved.path().join(format!("{name}-{role}")),
                        &devenv_core::store::StorePath::from(Path::new(package)),
                    )
                    .await?;
            }
        }
    ...
    /// Read durable target state without forcing any machine build outputs.
    pub async fn machines_status(&self, names: &[String]) -> Result<String> {
        let meta = self.load_machines_meta().await?;
        let selected: Vec<&str> = if names.is_empty() {
            meta.iter()
                .filter(|(_, m)| m.has_nixos && m.target.host.is_some())
        ...
        for name in selected {
            let machine = meta
                ...
                .ok_or_else(|| miette!("Unknown machine: {name}"))?;
            let host = machine.target.host.as_deref().ok_or_else(|| {
                miette!("machines.{name} requires target.host for deployment status")
            })?;
            if !machine.has_nixos {
                bail!("Deployment status requires a NixOS role on machines.{name}");
            }
            let target = SshTarget::parse(host)?;
            let script = format!(
                "test \"$(id -u)\" = 0 || {{ echo 'Deployment status requires root SSH' >&2; exit 1; }}; \
                 if test -x {MACHINE_EXECUTOR}; then {MACHINE_EXECUTOR} status; \
                 elif test -e /var/lib/devenv-machines/current.json; then \
                 echo 'Recorded deployment executor is unavailable' >&2; exit 1; \
                 else printf '%s\\n' '{{\"version\":1,\"phase\":\"uninitialized\"}}'; fi"
            );
            let output = self
                .executor_ssh_capture(&target, &machine.target.ssh_opts, &script)
                .await?;
            result.insert(
                name,
                serde_json::from_str::<serde_json::Value>(&output).into_diagnostic()?,
            );
        }
        Ok(format!(
            "{}\n",
    ...
    pub async fn machines_rollback(&self, name: &str) -> Result<()> {
        let meta = self.load_machines_meta().await?;
        let machine = meta
            .get(name)
            .ok_or_else(|| miette!("Unknown machine: {name}"))?;
        if !machine.has_nixos {
            bail!("Rollback requires a NixOS role on machines.{name}");
        }
        let host = machine
            .target
            .host
            .as_deref()
            .ok_or_else(|| miette!("machines.{name} requires target.host for rollback"))?;
        self.run_machine_transaction(
            name,
            &SshTarget::parse(host)?,
            &machine.target.ssh_opts,
            Path::new(MACHINE_EXECUTOR),
            None,
            None,
        )
        .await
    ...
    async fn run_machine_transaction(
        &self,
        name: &str,
        target: &SshTarget,
        ssh_opts: &[String],
        ...
        // Generate the ID locally so a lost submission response still has a
        // known identity. The temporary file carries no deployment contents.
        ...
        let id = request
            .path()
            .file_name()
            .ok_or_else(|| miette!("Could not allocate a deployment ID"))?
            .to_string_lossy();
        let executable = shell_quote(&executable.display().to_string());
        let mut command = match toplevel {
            Some(path) => format!(
                "{executable} start {} {}",
                shell_quote(&id),
                shell_quote(&path.display().to_string())
            ),
            None => format!("{executable} rollback {}", shell_quote(&id)),
        };
        if let Some((running, profile)) = expected {
            command.push_str(&format!(
                " --expected-system {} --expected-profile {}",
                shell_quote(running),
                shell_quote(profile)
            ));
        }
        let activity = activity!(INFO, operation, format!("NixOS transaction {id}"));
        async {
            self.executor_ssh_capture(target, ssh_opts, &command)
                .await
                .wrap_err_with(|| format!(
                    "Could not acknowledge transaction {id} on {name}. It may have started; \
                     inspect `devenv machines status {name}` before retrying."
                ))?;

            let deadline = tokio::time::Instant::now() + std::time::Duration::from_secs(900);
            loop {
                let output = self.executor_ssh_capture(target, ssh_opts, &format!("{executable} status"))
                    .await
                    .wrap_err_with(|| format!(
                        "Lost observation of transaction {id} on {name}. Activation runs on the target; \
                         reconnect with `devenv machines status {name}`."
                    ))?;
                let status: serde_json::Value = serde_json::from_str(&output).into_diagnostic()?;
                if status["version"].as_u64() != Some(1) {
                    bail!("Unsupported executor status version on {name}; inspect the target before retrying");
                }
                if status["id"].as_str() != Some(id.as_ref()) {
                    bail!("Target reports a different transaction; inspect `devenv machines status {name}`");
                }
                match status["outcome"].as_str() {
                    Some("succeeded") => return Ok(()),
                    Some("rolled-back") => bail!(
                        "Transaction {id} was rolled back on {name}: {}",
                        status["error"].as_str().unwrap_or("deployment was not confirmed")
                    ),
                    Some("rollback-failed") => bail!(
                        "Transaction {id} failed and automatic rollback failed on {name}: {}. Inspect `devenv machines status {name}` before recovery.",
                        status["rollbackError"].as_str().unwrap_or("previous system could not be restored")
                    ),
                    Some("failed") => bail!(
                        "Transaction {id} failed on {name}: {}. Inspect \
                         `devenv machines status {name}` before deciding whether to roll back or create a new plan.",
                        status["error"].as_str().unwrap_or("activation failed")
                    ),
                    Some("pending") => {
                        if status["phase"].as_str() == Some("awaiting-confirmation") {
                            // A fresh SSH round trip after target-side health checks
                            // confirms that the controller can reach this generation.
                            self.executor_ssh_capture(target, ssh_opts,
                                &format!("{executable} confirm {}", shell_quote(&id)))
                                .await
                                .wrap_err_with(|| format!(
                                    "Could not acknowledge confirmation of {id}; inspect `devenv machines status {name}`"
                                ))?;
                        }
                    },
                    _ => bail!("Transaction {id} has an unknown outcome; inspect `devenv machines status {name}`"),
                }
                if tokio::time::Instant::now() >= deadline {
                    bail!("Timed out observing transaction {id}; it may still be running. Inspect `devenv machines status {name}`");
                }
    ...
    async fn executor_ssh_capture(
        ...
        tokio::time::timeout(
            ...
            self.ssh_run_capture(target, ssh_opts, script),
        )
        .await
        .map_err(|_| {
...
mod tests {
    #[test]
    fn preview_observation_preserves_profile_drift_and_deduplicates_closure() {
        let running = "/nix/store/00000000000000000000000000000000-system";
        let profile = "/nix/store/11111111111111111111111111111111-system";
        let (observed, selected, closure) = super::parse_plan_observation(&format!(
            "devenv-machine-plan-v1\n{running}\n{profile}\n{running}\n{running}\n"
        ))
        .unwrap();
    ...
    #[test]
    fn preview_rejects_incomplete_or_invalid_target_observations() {
        let system = "/nix/store/00000000000000000000000000000000-system";
        for response in [
            String::new(),
            format!("login banner\ndevenv-machine-plan-v1\n{system}\n{system}\n{system}\n"),
            format!("devenv-machine-plan-v1\n{system}\n{system}\n"),
            format!("devenv-machine-plan-v1\n/tmp/system\n{system}\n{system}\n"),
            format!("devenv-machine-plan-v1\n{system}\n{system}\n{system}\nnot-a-store-path\n"),
        ] {
            assert!(
                super::parse_plan_observation(&response).is_err(),
                "{response:?}"
            );
        }
    }
    ...
    #[test]
    fn preview_validates_nix_store_path_boundaries() {
        for path in [
            "/nix/store/00000000000000000000000000000000-",
            "/nix/store/0000000000000000000000000000000-system",
            "/nix/store/eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee-system",
            "/nix/store/00000000000000000000000000000000-system/child",
            "/nix/store/00000000000000000000000000000000-system with spaces",
        ] {
            assert!(super::validate_plan_store_path(path).is_err(), "{path}");
        }
        assert!(
            super::validate_plan_store_path(
                "/nix/store/00000000000000000000000000000000-libfoo-1.0+dev"
            )
            .is_ok()
        );
    }
    ...
    fn parse_bracketed_ipv6() {
        let t = SshTarget::parse("root@[2001:db8::1]").unwrap();
        assert_eq!(t.user.as_deref(), Some("root"));
        assert_eq!(t.host, "[2001:db8::1]");
        assert_eq!(t.port, None);
        assert_eq!(t.ssh_destination(), "root@[2001:db8::1]");
        assert_eq!(t.nix_copy_uri(), "ssh://root@[2001:db8::1]");
    }

    #[test]
    fn parse_bracketed_ipv6_with_port() {
        let t = SshTarget::parse("ssh://root@[2001:db8::1]:2222").unwrap();
        assert_eq!(t.user.as_deref(), Some("root"));
        assert_eq!(t.host, "[2001:db8::1]");
        assert_eq!(t.port, Some(2222));
        assert_eq!(t.ssh_destination(), "root@[2001:db8::1]");
        assert_eq!(t.nix_copy_uri(), "ssh://root@[2001:db8::1]:2222");
    }
    ...
    fn duplicate_install_machine_names_are_rejected() {
        ...
        let error = validate_unique_machine_names(&names, "install").unwrap_err();
    ...
    #[test]
    fn duplicate_deploy_machine_names_are_rejected() {
        let error =
            validate_unique_machine_names(&["server".to_string(), "server".to_string()], "deploy")
                .unwrap_err();
        assert!(
            error
                .to_string()
                .contains("Duplicate machine name(s) in deploy request")
        );
        assert!(error.to_string().contains("server"));
    }
    ...
    #[test]
    fn nix_ssh_opts_env_shell_quotes_each_argv_token() {
        let user = vec![
            "-o".to_string(),
            "ProxyCommand=ssh -W %h:%p bastion".to_string(),
            "-i".to_string(),
            "/tmp/team's identity".to_string(),
        ];
...
"#,
    ...
    #[test]
    fn opportunistic_cachix_lookup_only_runs_when_it_can_supply_a_missing_token() {
        let mut settings = SecretSettings {
        ...
        assert!(should_resolve_opportunistic_cachix_token(
        ...
        settings.secretspec.as_mut().unwrap().cachix_auth_token =
            Some(devenv_core::config::CachixAuthToken::Enabled(false));
        assert!(!should_resolve_opportunistic_cachix_token(
            &settings, true, true, false
        ));
    }
    ...
    fn resolve_builders_config_with_targets() {
        ...
        let machine = |host: Option<&str>, has_nixos: bool| MachineMeta {
        ...
        meta.insert(
            "server".to_string(),
            machine(Some("root@server.example.com"), true),
        );
        meta.insert(
            "server-alias".to_string(),
            machine(Some("ssh://builder@SERVER.EXAMPLE.COM:2222"), true),
        );
        meta.insert(
            "other".to_string(),
    ...
    #[test]
    fn machine_host_identity_normalizes_equivalent_targets() {
        let identity = |target: &str| SshTarget::parse(target).unwrap().host_identity();

        assert_eq!(identity("server"), identity("ssh://root@SERVER.:22"));
        assert_eq!(
            identity("root@[2001:0db8:0:0:0:0:0:1]"),
            identity("builder@[2001:db8::1]:2222")
        );
    }
}
