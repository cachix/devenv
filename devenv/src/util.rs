...
///
/// The lock is a blocking advisory lock that another devenv process can hold
/// indefinitely, so the work runs on a blocking thread rather than a tokio worker.
pub async fn write_file_with_lock(path: PathBuf, content: String) -> Result<bool> {
    ...
    .await
    .map_err(|e| miette!("Failed to spawn file write task: {}", e))?
}
