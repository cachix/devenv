...
impl Devenv {
    ...
    pub async fn prepare_shell(
        ...
        let mut shell_cmd = if config_sandbox.enable {
            self.create_sandboxed_command(&config_sandbox, &config_clean, &bash)
                .await?
        } else {
    ...
    async fn create_sandboxed_command(
        ...
        for m in &config_sandbox.mounts {
            let src = &util::expand_path(&m.path);
            let mount_point = if let Some(dest) = &m.dest {
                &util::expand_path(dest)
            } else {
                src
            };
            let (mount_type, dest) = match m.mode.clone().unwrap_or_default() {
                devenv_core::config::BindMountMode::ReadOnly => (
                    if m.optional {
                        "--ro-bind-try"
                    } else {
                        "--ro-bind"
                ...
                devenv_core::config::BindMountMode::ReadWrite => (
                    if m.optional { "--bind-try" } else { "--bind" },
                    Some(mount_point),
                ),
                devenv_core::config::BindMountMode::Device => (
                    if m.optional {
                        "--dev-bind-try"
                    } else {
                        "--dev-bind"
                ...
                devenv_core::config::BindMountMode::Tmpfs => ("--tmpfs", None),
                devenv_core::config::BindMountMode::Overlay => {
                    ...
                    if !std::fs::exists(&overlay_work_dir)
                        .expect("Failed to check if sandbox overlay directory exists")
                    {
                        std::fs::create_dir_all(&overlay_rw_dir)
                            .expect("Failed to create sandbox overlay directory");
                        std::fs::create_dir(&overlay_work_dir)
                            .expect("Failed to create sandbox overlay directory");
                    }

                    bwrap.arg("--overlay-src").arg(src);
                    bwrap
                        .arg("--overlay")
                        .arg(&overlay_rw_dir)
                ...
                devenv_core::config::BindMountMode::OverlayTmpfs => {
                    bwrap.arg("--overlay-src").arg(src);
                    ("--tmp-overlay", Some(mount_point))
                }
                devenv_core::config::BindMountMode::OverlayRo => {
                    bwrap.arg("--overlay-src").arg(src);
                    ("--ro-overlay", Some(mount_point))
                }
            };
            bwrap.arg(mount_type).arg(src);
    ...
    fn add_bash_args(
        ...
        match cmd {
            ...
            None => {
                command.args(["--rcfile", &script_path.to_string_lossy()]);
            }
        }
    }
