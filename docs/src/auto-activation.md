...
The hook will not activate if the `DEVENV_ROOT` environment variable is set, which means it will not try to nest environments when you are already inside a `devenv shell`.
