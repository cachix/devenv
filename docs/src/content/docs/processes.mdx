...
Wait for all processes to become ready (useful in CI). The native manager and
process-compose both support `devenv processes wait` and `devenv processes list`:
...
| process-compose | Yes | No | Yes | No | Yes |
...
| process-compose | `none` | `process-scope` | `process-compose` |
...
uses devenv's native manager socket; `process-compose` uses the process-compose CLI over its control socket for
`devenv processes list` and `devenv processes wait`; `none` means those capabilities must remain disabled.
