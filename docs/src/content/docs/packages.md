...
`inputs."nixpkgs-multiverse"` for advanced use. See [Pinning an individual package version](/pinning/#pinning-an-individual-package-version)
...
[the raw input](/inputs/) exposes it to Nix as data. See
...
[fetch it from another nixpkgs input](/recipes/nix/).
