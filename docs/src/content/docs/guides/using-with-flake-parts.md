---
title: "Using devenv with flake-parts"
---
...
See [`devenv.nix` options](/reference/options/) for more information about configuration options.
...
For example, if `devenv-foo.nix` declares a devenv [service](/services/), and you've packaged it locally into [`perSystem.packages`](https://flake.parts/options/flake-parts.html#opt-perSystem.packages), you can provide the package as follows:
...
Here we have defined two shells, each with a `devenv` configuration and differently defined [`enterShell`](/reference/options/#entershell) command.
