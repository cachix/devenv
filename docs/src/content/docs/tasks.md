...
Processes are tasks too (see [Processes as tasks](#processes-as-tasks)), so the same `before`/`after` edges connect tasks and processes interchangeably — see [process dependencies](/processes/#dependencies) for process-focused examples.
...
| `@ready` | a process passes its [readiness probe](/processes/#ready-probes); for oneshot tasks this means success | yes |
...
<small class="added-in">Added in <code>1.4</code></small>
...
:::caution[Setup tasks attached to processes and `devenv up`]
