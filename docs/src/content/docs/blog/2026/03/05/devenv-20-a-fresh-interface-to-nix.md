---
title: "devenv 2.0: A Fresh Interface to Nix"
date: 2026-03-05
authors:
  - name: "Domen Kožar"
...
Every devenv command now shows a live terminal interface. Instead of scrolling Nix build logs, you see structured progress: what Nix is evaluating, how many derivations need to be built and downloaded, [task](/tasks/) execution with dependency hierarchy, and error details that expand automatically on failure.
...
direnv isn't needed anymore with `devenv shell` but it's still supported for automatic activation when switching directories; see the [direnv integration](/integrations/direnv/)
...
This builds on the existing [monorepo](/guides/monorepo/) support and extends it to multi-repository workflows. See the [polyrepo guide](/guides/polyrepo/) for full documentation.
...
Works with `devenv shell`, [`devenv test`](/tests/), and `devenv build`. Currently `--from` only works with projects that use `devenv.nix` alone; [projects that also rely on `devenv.yaml` for extra inputs aren't supported yet](https://github.com/cachix/devenv/issues/2205).
...
For a step by step upgrade guide, see [Migrating to devenv 2.0](/guides/migrating-to-20/).

- The [`git-hooks`](/git-hooks/) input is no longer included by default. If you use `git-hooks.hooks`, add it to your `devenv.yaml`.
