...
`devenv tasks run` starts from the requested task and its dependencies, so it does not run `devenv:enterShell` automatically.
If a task needs shell setup, add an `after` dependency on the setup task so its environment exports reach your task.
For an example, see [using a Python virtual environment in tasks](/languages/python/#using-the-virtual-environment-in-tasks).
...
<VersionCompatibility version="1.4" />
...
!!! note "Processes only live for the duration of the run"
    ...
    Use [`devenv up`](/processes/) when you want a process to keep running.
...
This runs when `app:cleanup` is selected in a task run and the application server finishes. For cleanup when the entire native `devenv up` session stops, use `devenv:up@stopped` instead:
...
`devenv up` runs this task after stopping all processes. It runs for foreground shutdown and `devenv processes down` after `devenv up -d`. It does not run when only one process is stopped while the manager remains active. Cleanup tasks can use outputs from startup tasks and depend on one another. The native manager validates their dependencies before starting processes, and `devenv processes down` reports cleanup failures. This dependency is supported by the native manager in devenv 2.4.1 and later.
...
<VersionCompatibility version="1.10" />
