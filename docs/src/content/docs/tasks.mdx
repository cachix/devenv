...
`devenv up` runs this task after stopping all processes. It runs for foreground shutdown and `devenv processes down` after `devenv up -d`. It does not run when only one process is stopped while the manager remains active. This dependency is supported by the native manager in devenv 2.4.1 and later.
