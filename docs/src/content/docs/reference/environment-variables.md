...
- [devenv variables](#devenv-variables) are defined by devenv.
  The read-only ones are exported into the developer shell so you can reference them from scripts, [tasks](/tasks/), and modules.
...
<small class="added-in">Added in <code>0.5</code></small>
...
See [Package outputs](/packages/#package-outputs) for which outputs of a package are linked into it.

### [`DEVENV_HOME`](#devenv_home)
<small class="added-in">Added in <code>2.1.3</code></small>
...
Override the provider for the [secretspec integration](/integrations/secretspec/).
...
Override the profile for the [secretspec integration](/integrations/secretspec/).
