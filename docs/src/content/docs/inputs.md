...
See [basics](/basics/) for more about `devenv.nix`.
...
- `pkgs` is a `nixpkgs` input containing [all of the available packages](/packages/#searching) for your system.
- `multiverse`, when the `nixpkgs-multiverse` input is configured, provides packages indexed by version, such as
  ...
  [the fewest nixpkgs revisions](/packages/#pinning-several-packages) and returns the packages. The original input
...
- `config` is the final resolved configuration for your developer environment, which you can use to reference any other options set in [devenv.nix](/reference/options/).
...
To update an input to a newer commit, run `devenv update` or read the [devenv.yaml reference](/reference/yaml-options/) to learn how to pin down the revision/branch at the input level.
