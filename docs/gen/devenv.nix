...
let
  ...
  sources = [
    {
      ...
      name = lib.strings.removeSuffix "/src/modules" (toString inputs.devenv.outPath);
  ...
  rewriteSource =
    decl:
    let
      source = lib.lists.findFirst (src: lib.strings.hasPrefix src.name decl) { } sources;
      path = lib.strings.removePrefix source.name decl;
